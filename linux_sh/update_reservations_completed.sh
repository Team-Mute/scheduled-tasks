...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/update_reservations_completed.log"

echo "[$(date '+%F %T')] Start (completed)" | tee -a "$LOG_FILE"
...
# psql 실행 (.pgpass 사용 권장)
# 먼저 인코딩 설정 후, SQL 파일 실행
...
psql "host=$DBHOST port=$DBPORT dbname=$DBNAME user=$DBUSER $SSL_OPT" \
  -v ON_ERROR_STOP=1 -f "$COMPLETE_SQL_FILE" >> "$LOG_FILE" 2>&1

echo "[$(date '+%F %T')] Done (completed)" | tee -a "$LOG_FILE"